...
function escucharPuntos(uid) {
    onSnapshot(doc(db, "usuarios", uid), (d) => {
        if(d.exists()){
            ...
            let nom = d.data().nombre || "Usuario";
            document.getElementById('saludoUsuario').innerText = nom.split(" ")[0];
...
// ==========================================
// ESCÁNER (AUTO-ZOOM, ALTA RESOLUCIÓN Y MODO DIOS)
// ==========================================
document.getElementById('btnAbrirScanner').addEventListener('click', () => {
    ...
    html5QrCode.start(cameraConfig, qrConfig, async (decodedText) => {
        ...
        // Intentamos hacer un zoom de 2.5x para enfocar la pantallita OLED
        setTimeout(() => {
            try {
                const track = html5QrCode.getRunningTrack();
                const capabilities = track.getCapabilities();
                if (capabilities.zoom) {
                    const zoomDeseado = Math.min(capabilities.zoom.max, 2.5);
                    track.applyConstraints({ advanced: [{ zoom: zoomDeseado }] });
                }
            } catch (e) {
                console.log("Auto-zoom no soportado en este dispositivo.");
            }
        }, 500);
    }).catch(err => {
