...
// ==========================================
// DATOS EN TIEMPO REAL E HISTORIAL
// ==========================================
...
// ==========================================
// ESCÁNER MODO LIBRE (CUALQUIER QR SUMA 10 PTS)
// ==========================================
...
async function procesarQR(textoQR) {
    // 1. Limpiamos cualquier carácter raro para que Firebase no se enoje al guardarlo como ID
    const identificadorSeguro = textoQR.replace(/[^a-zA-Z0-9]/g, "_").substring(0, 30);

    try {
        // 2. Verificamos si este QR específico ya fue escaneado antes en tu app
        const qrRef = doc(db, "transacciones_globales", identificadorSeguro);
        ...
        if (qrDoc.exists()) {
            Swal.fire({ 
                icon: 'warning', 
                title: 'Código Usado', 
                text: 'Este código QR ya fue registrado anteriormente.',
                confirmButtonColor: '#F1C40F'
            });
            document.querySelectorAll('.screen').forEach(s => s.classList.remove('active'));
            document.getElementById('dashboard-section').classList.add('active');
        } else {
            // 3. Si es nuevo, damos los puntos
            const fechaActual = new Date().toLocaleString();
            
            // Guardamos a nivel global para que no lo puedan volver a escanear
            await setDoc(qrRef, { usadoPor: currentUserUid, fecha: fechaActual, qrOriginal: textoQR });
            
            // Guardamos en el historial del alumno
            await addDoc(collection(db, `usuarios/${currentUserUid}/mi_historial`), {
                idTransaccion: "QR_" + identificadorSeguro.substring(0, 8),
                fecha: fechaActual, 
                puntos: 10
            });
            
            // Sumamos los 10 puntos en vivo a su perfil
            ...
            Swal.fire({
                icon: 'success',
                title: '+10 Eco-Puntos',
                text: `¡QR detectado y canjeado con éxito!`,
